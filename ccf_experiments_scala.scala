// CCF Experiments for spark-shell
// Run with:
//   spark-shell -i ccf_experiments_scala.scala
// sc and spark are pre-provided by spark-shell
...
def generateRandomGraph(nNodes: Int, nEdges: Int, seed: Int = 42): Seq[(String, String)] = {
  ...
  val edges = mutable.LinkedHashSet[(String, String)]()
...
def runCCF(edges: Seq[(String, String)], useSecondarySort: Boolean,
           maxIterations: Int = 100): (Int, Double, Int) = {
...
for ((nNodes, nEdges) <- randomConfigs) {
  ...
  for ((name, useSecSort) <- Seq(("Basic", false), ("SecondarySort", true))) {
    val (iters, elapsed, nComp) = runCCF(edges, useSecSort)
...
val pw = new PrintWriter(csvPath)
